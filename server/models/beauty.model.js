...
beautySchema.virtual("postedTime").get(function () {
  const now = new Date();
...
beautySchema.index({ status: 1, createdAt: -1 });
beautySchema.index({ seller: 1, status: 1 });
beautySchema.index({ price: 1 });
beautySchema.index({ savedBy: 1 });
beautySchema.index({
  title: "text",
  description: "text",
  brand: "text",
...
attachListingVideosField(beautySchema);

...
const otherSchema = new mongoose.Schema(
  {
    ...
    subcategory: {
      ...
      enum: {
        values: ["Other Items"],
        message: "Invalid subcategory for Others",
      },
    },
    condition: {
      type: String,
...
otherSchema.virtual("postedTime").get(function () {
  const now = new Date();
...
otherSchema.index({ status: 1, createdAt: -1 });
otherSchema.index({ seller: 1, status: 1 });
otherSchema.index({ price: 1 });
otherSchema.index({ savedBy: 1 });
otherSchema.index({ title: "text", description: "text" });
otherSchema.index({ coordinates: "2dsphere" });

attachListingVideosField(otherSchema);
attachSlugPlugin(otherSchema);

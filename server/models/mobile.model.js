...
const mobileSchema = new mongoose.Schema(
  {
    ...
    subcategory: {
      ...
      enum: {
        ...
        message: "Invalid subcategory for Mobiles",
      },
    },
    condition: {
      type: String,
...
mobileSchema.virtual("postedTime").get(function () {
  const now = new Date();
...
mobileSchema.index({ status: 1, createdAt: -1 });
mobileSchema.index({ seller: 1, status: 1 });
mobileSchema.index({ price: 1 });
mobileSchema.index({ savedBy: 1 });

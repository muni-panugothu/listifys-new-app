...
const createEventSchema = Joi.object({
  ...
  startTime: Joi.string().trim().max(80).allow(''),
  endTime: Joi.string().trim().max(80).allow(''),
  organizer: Joi.string().trim().max(200).allow(''),
  venue: Joi.string().trim().max(300).allow(''),
  ticketsAvailable: Joi.number().integer().min(1).required().messages({
    'number.min': 'Total tickets must be at least 1',
    'any.required': 'Total tickets available is required',
  }),
  ...
  eventCategory: Joi.string().trim().max(80).allow(''),
  eventType: Joi.string().trim().max(80).allow(''),
  categoryData: Joi.object().unknown(true).optional(),
  status: Joi.string().valid('draft', 'active', 'published').optional(),

...
function Convert-ChromaIcon([string]$Source, [string]$Dest) {
  if (-not (Test-Path $Source)) { Write-Output "MISSING $Source"; return }
...
$destDir = 'C:\Users\Dell\listifys-app\app\assets\home\explore\categories'
$ids = @(
  'electronics','vehicles','mobiles','furniture','fashion','jobs',
  'takecare','events','services','properties','sports','collectibles',
  'pets','books','beauty','others','toys'
)
foreach ($id in $ids) {
  Write-Output "Chroma $id..."
  Convert-ChromaIcon -Source (Join-Path $srcDir "cat-3d-$id.png") -Dest (Join-Path $destDir "cat-$id.png")
}
Write-Output 'DONE'

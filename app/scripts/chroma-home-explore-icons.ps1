...
$ids = @('dining', 'movies', 'events', 'stores', 'activities', 'play')

foreach ($id in $ids) {
  $src = Join-Path $srcDir "home-explore-$id.png"
  $dest = Join-Path $destDir "explore-$id.png"
  Write-Output "Chroma $id..."
  Convert-ChromaIcon -Source $src -Dest $dest
  $img = [System.Drawing.Bitmap]::FromFile($dest)
  Write-Output ("  {0}x{1} cornerA={2}" -f $img.Width, $img.Height, $img.GetPixel(2,2).A)
  $img.Dispose()
}
Write-Output 'DONE'
